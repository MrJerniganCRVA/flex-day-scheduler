 ...
 * they are on, and the rosters they have to take.
 *
 * Lived inline on the teacher dashboard until admins needed it too. An admin can
 * own a club and be assigned a duty post exactly like a teacher — the Coverage
 * page picks teachers with `role: { in: ["TEACHER","ADMIN"] }` — but the admin
 * dashboard only ever showed school-wide placement stats, so an admin on hallway
 * duty had to visit /teacher to find that out. Rather than a second, drifting
 * copy of the same three columns, both dashboards render this.
 */
...
async function loadFlexDay(userId: string, flexDayId?: string) {
  ...
  if (flexDayId) {
    return prisma.flexDay.findUnique({
      where: { id: flexDayId },
      include: { clubSessions: { where, include: SESSION_INCLUDE } },
    });
  }
...
type LoadedFlexDay = NonNullable<Awaited<ReturnType<typeof loadFlexDay>>>;
type MySession = LoadedFlexDay["clubSessions"][number];

export default async function MyFlexDay({
  userId,
  flexDayId,
  newSessionHref,
  hideWhenEmpty = false,
  ...
  userId: string;
  /**
   * Skip the "soonest active day" lookup and use this day instead. The admin
   * dashboard has already resolved it for its school-wide card, and passing it
   * down is what stops the two halves of that page describing different days.
   */
  flexDayId?: string;
  /** Where "+ Schedule a session for this day" points. Omitted hides the link. */
  newSessionHref?: string;
  /** Render nothing when there is no upcoming Flex Day, instead of an empty state. */
  hideWhenEmpty?: boolean;
  /**
   * Show the "Next Flex Day" eyebrow and the date. Off for a caller that has
   * already named the day directly above — the admin dashboard would otherwise
   * print the same date three times down one screen.
   */
  showDayHeader?: boolean;
}) {
  const nextFlexDay = await loadFlexDay(userId, flexDayId);
  ...
  // Duty posts this person is on for that day. Read-only here: someone who
  ...
  const myDuties = await prisma.dutyAssignment.findMany({
  ...
  const iAmAbsentFrom = (cs: MySession, slot: RotationSlot) =>
    cs.teacherAbsences.some((a) => a.teacherId === userId && a.rotation === slot);

  // Someone expected by two sessions in the same rotation cannot attend both.
  //
  ...
  // resolving coverage itself. Filtered to this person because the query above
  // is already scoped to sessions they are attached to.
  //
  // Absences are subtracted inside resolveSessionCoverage, so a session they
  // have already stepped back from still shows here (they can undo it) but stops
  // counting toward a clash.
  const sessionsById = new Map(nextFlexDay.clubSessions.map((cs) => [cs.id, cs]));
  const clashes = new Map<RotationSlot, MySession[]>();
  for (const clash of findTeacherClashes(
    nextFlexDay.clubSessions.map(sessionPlacement),
    ALL_ROTATIONS as RotationSlot[]
  )) {
    if (clash.teacherId !== userId) continue;
    clashes.set(
      clash.rotation,
      clash.placements
        .map((p) => sessionsById.get(p.id))
        .filter((cs): cs is MySession => cs !== undefined)
    );
  }
